...
public class DownLoadFileManager {
        ...
        ThreadPoolTool.getInstance().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    ShowLog.e(TAG, "initTempFilePercent-->" + e);
                }
            }
        });
    }
     ...
     * @return
     */
    public boolean isExistsFileByWhich(final String strDownloadFileName, final String strDownloadDir, int which) {
        File file = new File(strDownloadDir + strDownloadFileName);
        File tempFile = new File(strDownloadDir + strDownloadFileName + ".tmp" + which);
        if (file.exists() && !tempFile.exists()) {
            return true;
